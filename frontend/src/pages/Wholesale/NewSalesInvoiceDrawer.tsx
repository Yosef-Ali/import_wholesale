...
export default function NewSalesInvoiceDrawer({ onClose }: Props) {
  const [form, setForm] = useState<Record<string, any>>({ status: 'Draft', grand_total: 0, outstanding_amount: 0 });
      ...
      <div className="fixed top-0 right-0 bottom-0 w-[600px] bg-[var(--card)] border-l border-[var(--border)] rounded-l-2xl shadow-2xl z-50 flex flex-col overflow-hidden animate-in slide-in-from-right-10 duration-300">
        ...
        <div className="px-6 pt-6 pb-5 border-b border-[var(--border)] flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="font-primary text-xl font-bold text-[var(--foreground)] leading-tight">New Sales Invoice</h2>
              <p className="font-secondary text-xs text-[var(--muted-foreground)] mt-0.5">Create a new billing document</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          <form id="new-si-form" onSubmit={handleSubmit} className="space-y-5">

            <div className="flex items-center gap-3 mb-2">
              <span className="w-[3px] h-4 rounded-full bg-[var(--primary)] shrink-0" />
              <span className="font-primary text-[0.65rem] font-bold uppercase tracking-widest text-[var(--muted-foreground)]">Invoice Details</span>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" className={inputClass} value={form.grand_total || ''} onChange={e => setForm({ ...form, grand_total: parseFloat(e.target.value), outstanding_amount: parseFloat(e.target.value) })} />
              </div>
              <div>
        ...
        <div className="px-6 py-4 border-t border-[var(--border)] flex items-center justify-end gap-3 shrink-0 bg-[var(--background)]/40">
          ...
          <button
            type="submit" form="new-si-form" disabled={createInvoice.isPending}
