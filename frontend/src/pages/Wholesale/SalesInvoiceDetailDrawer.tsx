...
export default function SalesInvoiceDetailDrawer({ editName, onClose }: Props) {
  ...
  const handleDelete = async () => {
    if (!window.confirm(`Are you sure you want to delete Sales Invoice ${editName}?`)) return;
    try {
    ...
    <>
      ...
      <div className="fixed top-2 right-2 bottom-2 w-[550px] bg-[var(--card)] border border-[var(--border)] rounded-xl shadow-2xl z-50 flex flex-col overflow-hidden animate-in slide-in-from-right-8 duration-200">
        ...
        <div className="px-5 py-4 border-b border-[var(--border)] flex items-center justify-between shrink-0 bg-[var(--background)]/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-secondary text-xs text-[var(--muted-foreground)]">{item?.customer_name || item?.customer || 'Sales Invoice'}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {!isEditing ? (
              <>
                ...
                <a
                  href={`http://localhost:8081/api/method/frappe.utils.print_format.download_pdf?doctype=Sales%20Invoice&name=${editName}`}
                  target="_blank"
                  rel="noreferrer"
              ...
              <div className="flex items-center gap-1">
                ...
                <button 
                  ...
                  className="px-3 py-1.5 bg-blue-600 text-white text-xs font-semibold rounded-md hover:bg-blue-700 transition-colors flex items-center gap-1 cursor-pointer"
                >
                  {updateInvoice.isPending ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />} Save
                </button>
              </div>
            )}
          </div>
        ...
        <div className="flex items-center px-4 border-b border-[var(--border)] shrink-0 bg-[var(--background)]/30">
          <button onClick={() => setActiveTab('details')} className={`px-4 py-3 text-xs font-secondary font-semibold uppercase tracking-wider border-b-2 transition-colors ${activeTab === 'details' ? 'border-transparent text-blue-500' : 'border-transparent text-[var(--muted-foreground)] hover:text-[var(--foreground)]'}`}>Details</button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {isLoading ? (
            ...
            <div className="flex items-center justify-center h-full text-[var(--muted-foreground)] font-secondary text-sm">Invoice not found</div>
          ) : (
                ...
                <div className="grid grid-cols-2 gap-4">
                  {isEditing ? (
                    <>
                      ...
                      <div><span className="text-xs font-bold text-[var(--muted-foreground)] block mb-1">Posting Date</span><input type="date" className={inputClass} value={form.posting_date || ''} onChange={e => setForm({...form, posting_date: e.target.value})} /></div>
                      <div><span className="text-xs font-bold text-[var(--muted-foreground)] block mb-1">Due Date</span><input type="date" className={inputClass} value={form.due_date || ''} onChange={e => setForm({...form, due_date: e.target.value})} /></div>
                      ...
                      <div><span className="text-xs font-bold text-[var(--muted-foreground)] block mb-1">Outstanding</span><input type="number" className={inputClass} value={form.outstanding_amount || 0} onChange={e => setForm({...form, outstanding_amount: parseFloat(e.target.value)})} /></div>
                    </>
                  ) : (
                    <>
        ...
        <div className="px-5 py-3.5 border-t border-[var(--border)] flex items-center justify-between bg-[var(--background)]/50">
          ...
          <div className="flex gap-2">
            ...
            <a
              href={`http://localhost:8081/app/sales-invoice/${editName}`}
              target="_blank"
              rel="noreferrer"
